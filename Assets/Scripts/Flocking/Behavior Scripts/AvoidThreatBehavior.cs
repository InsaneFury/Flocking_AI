...
[CreateAssetMenu(menuName = "Flock/Behavior/AvoidThreatBehavior")]
public class AvoidThreatBehavior : FilterFlockBehaviour
{
    public float ThreatAvoidRadius = 5f;
    public override Vector3 CalculateMove(FlockAgent agent, List<Transform> context, Flock flock, Transform[] waypoints)
    {
        ...
        for (int i = 0; i < filteredContext.Count; i++)
        {
            ...
            if (Vector3.SqrMagnitude(closestPoint - agent.transform.position) < flock.GetSquareAvoidanceRadius + ThreatAvoidRadius)
            {
                nAvoid++;
